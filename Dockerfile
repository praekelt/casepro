...
#Upgrade Nodejs to v6 for default function parameters
#Check https://node.green for compatibility
RUN apt-get update && apt-get-install.sh git redis-server supervisor libpq-dev gcc curl -y && curl -sL https://deb.nodesource.com/setup_6.x | bash && apt-get install nodejs -y
